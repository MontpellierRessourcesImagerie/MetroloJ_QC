  ...
  public resolutionCalculator(int type, double emWavelength, double exWavelength, double NA, double refractiveIndex, Double pinhole) {
    ...
    switch (type) {
      case WIDEFIELD:
        ...
        break;
      case CONFOCAL:
        meanWavelength = Math.sqrt(2.0D) * emWavelength * exWavelength / Math.sqrt(Math.pow(exWavelength, 2.0D) + Math.pow(emWavelength, 2.0D));
        if (pinhole.doubleValue() <= 0.25D) {
          this.resolution[this.X] = 0.37D * meanWavelength / NA;
        } else {
        ...
        if (pinhole.doubleValue() > 0.25D) {
          ...
          this.resolution[this.Z] = 0.64D * meanWavelength / (refractiveIndex - Math.sqrt(Math.pow(refractiveIndex, 2.0D) - Math.pow(NA, 2.0D)));
        } 
      ...
      case SPINNING:
        this.resolution[this.X] = 0.51D * emWavelength / NA;
        this.resolution[this.Y] = this.resolution[this.X];
        this.resolution[this.Z] = 1.0D * emWavelength / (refractiveIndex - Math.sqrt(Math.pow(refractiveIndex, 2.0D) - Math.pow(NA, 2.0D)));
        this.nyquist[this.X] = emWavelength / 4.0D * NA;
        this.nyquist[this.Y] = this.nyquist[this.X];
        this.nyquist[this.Z] = emWavelength / 2.0D * refractiveIndex * (1.0D - Math.cos(Math.asin(NA / refractiveIndex)));
        break;
