 ...
 */
   ...
   */
  public void getDetectorParameters(String name, double[] saturation, Double channelChoice, String [] dateInfo) {
    int base, rows = 0;
    ...
    temp[3][0] = new content("Detector", content.TEXT, 1, 2);
    ...
    int refRow = 5;
    if (!this.conversionFactor.isNaN()) {
      temp[5][0] = new content("Conversion Factor (e-/ADU)", content.TEXT, 1, 2);
      temp[5][1] = new content();
      temp[5][2] = new content("" + dataTricks.round(this.conversionFactor.doubleValue(), 2), content.LEFTTEXT);
      refRow = 6;
    } 
    if (this.channels.length == 1 || !channelChoice.isNaN()) {
      int channelToAnalyse;
      temp[refRow][0] = new content("Saturation", content.TEXT);
      ...
      if (this.channels[channelToAnalyse].isEmpty())this.channels[channelToAnalyse]="Channel"+channelToAnalyse;
      temp[refRow][1] = new content(""+this.channels[channelToAnalyse],content.TEXT);
      if (saturation[channelToAnalyse] == 0.0D) {
        temp[refRow][2] = new content("none", content.PASSED);
      } else {
        double sat = dataTricks.round(saturation[channelToAnalyse] * 100.0D, 1);
        if (sat == 0.0D) {
          temp[refRow][2] = new content("<0.1%", content.FAILED);
        } else {
          temp[refRow][2] = new content("" + sat + "%", content.FAILED);
        } 
      } 
    } else {
      ...
      for (int i = 0; i < this.channels.length; i++) {
        ...
        temp[refRow + i][1] = new content(""+this.channels[i], content.TEXT);
        if (saturation[i] == 0.0D) {
          temp[refRow + i][2] = new content("none", content.PASSED);
        } else {
          double sat = dataTricks.round(saturation[i] * 100.0D, 1);
          if (sat == 0.0D) {
            temp[refRow + i][2] = new content("<0.1%", content.FAILED);
          } else {
            temp[refRow + i][2] = new content("" + sat + "%", content.FAILED);
          } 
        } 
      } 
    } 
