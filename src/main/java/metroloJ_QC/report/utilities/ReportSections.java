...
public class ReportSections {
  ...
  public int TITLE = 1;
  public int TITLE2 = 2;
  public static final boolean BACKGROUND_COLOR=true;
  public static final boolean HIGHLIGHT_FIRST_ROW=true;
  public static final boolean HIGHLIGHT_FIRST_COL=true;
  public static final boolean NO_BACKGROUND_COLOR=true;
  public static final boolean NORMAL_FIRST_ROW=false;
  public static final boolean NORMAL_FIRST_COL=false;
/**
 ...
 */
    public PdfPTable wholeSection(String sectionTitle, int titleType, String text) {
 ...
 * (according to the content object).
     * @param highlightFirstRow a boolean to decide whether the content of the first row should be in bold font
     * @param highlightFirstCol a boolean to decide whether the content of the first column should be in bold font
 * @return A PdfPTable with the specified content and appearance.
 */
  public PdfPTable table(content[][] list, float widthPercentage, boolean choice, boolean highlightFirstRow, boolean highlightFirstCol) {
    ...
    for (int row = 0; row < list.length; row++) {
      for (int col = 0; col < (list[0]).length; col++) {
        Font font;
        int fontSize = 9;
        if ((row == 0 && highlightFirstRow) || (col == 0 && highlightFirstCol)) fontSize = 10; 
 ...
 * @return A PdfPTable with the specified content and appearance.
 */
  public PdfPTable table(content[][] list, float widthPercentage, float[] heights, boolean choice, boolean highlightFirstRow, boolean highlightFirstCol) {
    PdfPTable table = new PdfPTable((list[0]).length);
    table.getDefaultCell().setHorizontalAlignment(1);
    table.getDefaultCell().setVerticalAlignment(5);
    table.setWidthPercentage(widthPercentage);
    for (int row = 0; row < list.length; row++) {
      for (int col = 0; col < (list[0]).length; col++) {
        ...
        cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
        cell.setFixedHeight(heights[row]);
        switch ((list[row][col]).status) {
          case content.TEXT:
            cell.setBackgroundColor(new BaseColor(255, 255, 255, 41));
            table.addCell(cell);
 ...
 */
  public PdfPTable tableOfTables(content[][] list1, float width1, content[][] list2, float width2, float widthPercentage, boolean highlightFirstRow, boolean highlightFirstCol) throws DocumentException {
    PdfPTable mainTable = new PdfPTable(2);
    ...
    PdfPTable table1=table(list1, tempWidth, false, highlightFirstRow, highlightFirstCol );
    tempWidth=(widthPercentage*width2)/(width1+width2);
    PdfPTable table2=table(list2, tempWidth, false, highlightFirstRow, highlightFirstCol);
 ...
 */
  ...
  /**
 * Creates a PdfPTable for displaying panel XY, XZ and ZY views in a specified format next to a classical table
 * @param panelViews An array of ImagePlus objects representing the panel views.
 * panelviews[0] contains the XY view, panelviews[1] contains the YZ view, 
 * panelviews[2] contains the XZ view, 
 * @param type The type of the panel view, e.g., "coa" or "pp".
 ...
 * @throws DocumentException If there's an error while creating the PDF document.
 */
  public PdfPTable combinedTable(ImagePlus[] panelViews, String type, content[][] list, float widthPercentage, boolean choice, boolean highlightFirstRow, boolean highlightFirstCol) throws DocumentException {
    PdfPTable mainTable = new PdfPTable(2);
    mainTable.setKeepTogether(true);
    mainTable.setWidthPercentage(widthPercentage);
    PdfPTable panelViewTable = panelViewTable(panelViews, type);
    PdfPCell panelCell = new PdfPCell(panelViewTable);
    panelCell.setBorder(0);
    mainTable.addCell(panelCell);
    PdfPTable classicTable = table(list, 100f, choice, highlightFirstRow, highlightFirstCol);
    PdfPCell classicCell = new PdfPCell(classicTable);
    classicCell.setBorder(0);
    mainTable.addCell(classicCell);
    mainTable.setSpacingBefore(15.0F);
    return mainTable;
}
 ...
 * @throws DocumentException If there's an error while creating the PDF document.
 */
  public PdfPTable combinedTable(PdfPTable tableA, PdfPTable tableB, float widthPercentage) {
    PdfPTable mainTable = new PdfPTable(2);
    mainTable.setKeepTogether(true);
    mainTable.setWidthPercentage(widthPercentage);
    PdfPCell panelCell = new PdfPCell(tableA);
    panelCell.setBorder(0);
    mainTable.addCell(panelCell);
    PdfPCell classicCell = new PdfPCell(tableB);
    classicCell.setBorder(0);
    mainTable.addCell(classicCell);
    mainTable.setSpacingBefore(15.0F);
    return mainTable;
 ...
 */
  ...
  public static void setRowHeight(PdfPTable table, float height) {
        for (int row = 0; row < table.getRows().size(); row++) {
            PdfPCell[] cells = table.getRow(row).getCells();
            if (cells != null) {
