...
public static final String[] TYPES = new String[] { "WideField", "CLSM", "Spinning Disc Confocal", "Multiphoton" };
public static final String[] ABBREVIATED_TYPES=new String[] {"WIDEFIELD", "CLSM", "SPINNING", "MULTIPHOTON"};
// associated final variables corresponding to the TYPES variable
...
// theoretical dimension1, dimension2 and dimension3 theoretical values for testChannel i. 
...
// nyquist dimension1, dimension2 and dimension3 sampling interval values for testChannel i  
...
public content[][] microscopeParameters;
 ...
 */
  public microscope(Calibration cal, int dimensionOrder, int microtype, double[] emWavelengths, double NA, Double pinhole, double[] exWavelengths, double refractiveIndex, int bitDepth) {
    ...
    for (int i = 0; i < emWavelengths.length; i++) {
        this.resolutions.add(theoreticalResolutionCalculator.getResolution(microtype, emWavelengths[i], exWavelengths[i], NA, refractiveIndex, pinhole));
        this.nyquists.add(theoreticalResolutionCalculator.getNyquist(microtype, emWavelengths[i], exWavelengths[i], NA, refractiveIndex));
    } 
  ...
  public microscope duplicateGenericMicroscope(){
      ...
      content[][] temp=microscopeParameters;
      output.microscopeParameters=temp;
  return(output);
  }
 ...
 the summary table is saved as a 2D Content array into the class variable microscopeParameters
   */
  public void getMicroscopeInformationSummary(String name, MetroloJDialog mjd, double[] saturation, String [] dateInfo, Double[] coordinates) {
    ...
    boolean showBeadOriginalPosition=false;
    ...
    switch (mjd.reportType){
        ...
        case "pp":
        case "dp":
            if (mjd.multipleBeads && !coordinates[0].isNaN() && !coordinates[1].isNaN()) showBeadOriginalPosition=true;
            break;
    }
    ...
    temp[refRow][2] = new content("" + TYPES[this.microtype], content.LEFTTEXT, 1, cols-2);
    if (this.microtype == 1) {
   ...
   for (int i = 0; i < this.emWavelengths.length; i++) {
        ...
        refCol=2;
        if (showWavelengths){
            if (microtype==WIDEFIELD||microtype==SPINNING){
                temp[refRow+i][refCol]=new content("", content.BLANK);
                temp[refRow+i][refCol+1]=new content("" + this.emWavelengths[i], content.TEXT);
            }
            else if (microtype==CONFOCAL||microtype==MULTIPHOTON){
                temp[refRow+i][refCol]=new content("" + this.exWavelengths[i], content.TEXT);
                temp[refRow+i][refCol+1]=new content("", content.BLANK);
            }
            refCol+=2;
        }
        if(showSaturation) {
    ...
    if (mjd.debugMode) IJ.log("(in microscope>getMicroParameters) showBeadOriginalPosition: "+showBeadOriginalPosition);
    ...
    this.microscopeParameters = temp;
    if (mjd.debugMode) content.contentTableChecker(this.microscopeParameters, "microscopeInformationSummary (as given by microscope>getMicroscopeInformationSummary)");
  }
 ...
 the summary table is saved as a 2D Content array into the class variable microscopeParameters
   */
  public void getGenericMicroscopeParameters(MetroloJDialog mjd, String path, String[] saturationProportion, String [][] samplingProportion, int nReports) {
    boolean showNyquist=true;
    boolean showSaturation=true;
    boolean showWavelengths=true;
    if (mjd.debugMode) IJ.log("(in microscope>getGenericMicroscopeParameters) mjd type: "+mjd.reportType);
    switch (mjd.reportType){
        case "bfi" :
            //showNyquist=false;
            //showSaturation=true;
    ...
    if (samplingProportion==null)showNyquist=false;
    ...
    if(!showNyquist&&!showSaturation&&!showWavelengths) cols++;
    if (this.microtype == 1)rows++;
    if (mjd.debugMode) IJ.log("(in microscope>getGenericMicroscopeParameters) rows: "+rows+", cols: "+cols);
    content[][] temp = new content[rows][cols];
   ...
   refRow+=2;
    if (mjd.debugMode) IJ.log("(in microscope>getGenericMicroscopeParameters) refRow before channel values implementation: "+refRow);
   ...
   for (int i = 0; i < this.emWavelengths.length; i++) {
    ...
    this.microscopeParameters = temp;
    if (mjd.debugMode) content.contentTableChecker(this.microscopeParameters, "microscopeParameters (as given by microscope>getGenericMicroscopeParameters)");
  }
  /**
