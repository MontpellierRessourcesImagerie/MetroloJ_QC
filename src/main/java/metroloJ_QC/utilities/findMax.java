 ...
 */
    public int[] getAllCoordinates(ImagePlus ip, MetroloJDialog mjd) {
    int[] coord;
    if (ip.getNSlices() == 1) {
      coord = get2DCenter(ip);
    } else {
      int[] coord2D = get2DCenter((new sideViewGenerator(ip, mjd)).getXYview(sideViewGenerator.MAX_METHOD));
      coord = new int[3];
      coord[0] = coord2D[0];
      coord[1] = coord2D[1];
