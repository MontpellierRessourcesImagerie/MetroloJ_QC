...
public class LegacyHistogramSegmentation {
   // a boolean to use the log histogram values for histogram segmentation
 ...
 */
  public LegacyHistogramSegmentation(ImagePlus ip, boolean debugMode) {
    ...
    if (this.debugMode) {
        IJ.log("(in LegacyHistogramSegmentation) channel "+ip.getShortTitle()+ ": \nmax: "+max+", min: "+min+" found in full Z Stack");
    }
  }
  /**
   * Calculates class limits for histogram segmentation.
 ...
 */
  public int[] calcLimits(int nClasses, int maxIt, int epsilon, boolean log) {
    ...
    if (debugMode) {
        for (int i=0; i<limits.length; i++) IJ.log("limits["+i+"]: "+limits[i]);
    }
    return this.limits;
 ...
 */
  public ImagePlus getsegmentedImage(ImagePlus ip, int nClass) {
    ...
    ImagePlus dest = NewImage.createImage("segmented_" + ip.getTitle(), ip.getWidth(), ip.getHeight(), ip.getNSlices(), 8, 1);
